...
if not os.path.exists(CHUNK_PATH) or not os.path.exists(INDEX_PATH):
    print("Ошибка: не найдены данные (chunk.json или index.faiss)")
    exit(1)
...
Если пользователь спрашивает что-то техническое — используй контекст.
Если просто болтает — отвечай по-человечески, спроси, чем помочь.
Не навязывай темы.
Контекст (если есть):
{context}
""".strip()}
    ]
    messages.extend(history[-10:])  # вся история
